...
export function createPaginationLink(
  cache: InMemoryCache,
  paginatePolicies: Record<string, FieldPolicy>
): ApolloLink {
  return new ApolloLink((operation, forward) => {
    ...
    visit(query, {
      Directive(node, _key, _parent, path) {
        const foundDirective = possibleDirectives.find(
        ...
        // Clean Query
        operation.query =
          removeDirectivesFromDocument([{ name: foundDirective }], query) ??
          query;

        const field = get(
          query,
          path.join('.').replace(/\.directives\.0$/, '')
        );

        const typenameArg = node.arguments?.find(
          arg => arg.name.value === 'typename'
        );
        const typename =
          (typenameArg?.value as StringValueNode)?.value ?? 'Query';

        const keyArgsArg = node.arguments?.find(
          arg => arg.name.value === 'keyArgs'
        );
        const keyArgs =
